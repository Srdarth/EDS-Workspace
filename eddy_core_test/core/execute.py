...
def execute_plans(
    ...
    # Dry-run: registrar PLAN_MKDIR para pastas que terão arquivos
    if dry_run:
        parent_dirs = set()
    ...
    for idx, (pid, src, action, dest, reason) in enumerate(exec_cur, start=1):
        ...
        # Verificar canônico por conteúdo (content_index) — só se tabela existir
        cid = None
        try:
            row = conn.execute(
        ...
        # Verificar duplicata em content_index (só se tabela existir)
        canon = None
        canon_md5 = None
        if sh and _table_exists(conn, "content_index"):
            try:
        ...
        try:
            ...
            # Registrar canônico se content_index existir
            if _table_exists(conn, "content_index") and (sh or cid):
                try:
                    st = dest_p.stat()
